...
. "$(dirname "$0")/env.sh"
exec $TORCHRUN scripts/train_stage2_flowmap.py --results-dir "$RESULTS" --run-name s2_shifted_lagrangian --resume --on-keep-cmd "$PWD/runs/fid_on_keep.sh s2_shifted_lagrangian {step}" \
  $STAGE2_COMMON \
