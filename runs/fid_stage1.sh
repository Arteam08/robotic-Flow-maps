...
# Sampling runs in bf16 (the precision of every reference FID we quote; training precision is a separate choice).
...
. "$(dirname "$0")/env.sh"
N=${N:-2000}
if [ "$1" = "--ckpt" ]; then CKPT=$2; RUN=$3; STEP=0; else CKPT=$RESULTS/$1/checkpoints/step_$(printf %07d $2).pt; [ -f "$CKPT" ] || CKPT=$RESULTS/$1/kept/step_$(printf %07d $2).pt; RUN=$1; STEP=$2; fi
...
for W in raw ema; do for st in $STOPS; do
  ...
  $PY scripts/eval_fid.py --ckpt "$CKPT" $WFLAG --output-dir $d --num-samples $N --batch-size ${FID_BATCH:-50} --seed 0 \
    --num-steps 250 --eps-stop $eps --cfg-scale 1.5 --cfg-mode all --cfg-schedule constant \
    --cfg-match-uncond-norm --cfg-match-guided-norm --bf16 --skip-pytorch-fid --save-samples-npz $d/samples.npz
